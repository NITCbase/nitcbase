...
int project(char srcrel[ATTR_SIZE], char targetrel[ATTR_SIZE], int tar_nAttrs, char tar_attrs[][ATTR_SIZE]) {
	...
	if (srcrelid == E_RELNOTOPEN) {
		return E_RELNOTOPEN; // src relation not opened
	}
...
int join(char srcrel1[ATTR_SIZE], char srcrel2[ATTR_SIZE], char targetrel[ATTR_SIZE], char attr1[ATTR_SIZE],
	...
	while (ba_search(srcrelid1, rec1, "PROJECT", dummy, PRJCT, &prev_recid1) == SUCCESS) {
		...
		prev_recid2.block = -1;
		prev_recid2.slot = -1;
		while (ba_search(srcrelid2, rec2, attr2, rec1[attrcat_entry1[5].ival], EQ, &prev_recid2) ==
		       SUCCESS) {   //cout<<"beginning of hit"<<endl;
			for (int iter = 0; iter < nAttrs1; iter++)
