...
int main() {
	while (1) {
		...
		if (regex_match(input_command, ex)) {
			...
			/* Extract the Relation Name and get Relid */
			string_to_char(tablename, relname, 15);
			...
			/* Gaurd for Relation and Attribute Catalogue */
			if (id == 0 || id == 1) {
			...
			/* Extract Table and Attrbiute Names and Find Relid */
			string_to_char(tablename, relname, 15);
