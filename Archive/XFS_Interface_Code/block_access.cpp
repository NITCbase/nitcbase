...
int ba_search(relId relid, union Attribute *record, char attrName[ATTR_SIZE], union Attribute attrval, int op,
	...
	if ((recid.block == -1) && (recid.slot == -1)) { //if it fails to find a record satisfying the given condition
		return FAILURE;
	}
...
int ba_delete(char relName[ATTR_SIZE]) {
	...
	if ((relcat_recid.block == -1) && (relcat_recid.slot == -1)) { //If relation with relName does not exits
...
int ba_renameattr(char relName[ATTR_SIZE], char oldName[ATTR_SIZE], char newName[ATTR_SIZE]) {
	...
	if ((relcat_recid.block == -1) && (relcat_recid.slot == -1)) { //If relation with name relName does not exits
		return E_RELNOTEXIST;
	}
