...
void read_attr_cat(char *rel_name,int no_attributes,char *filename)
{
	printf("\n77777");
	printf("\nin readattrcat%s",filename);
	FILE *disk = fopen("disk.txt","r");
	...
	if(curr_blk==-1)
   		break;
	next_blk=fseek(disk,curr_blk*2048+12, SEEK_SET);
	}while(1);
...
void export(char *rel_name,char *filename)
{
	//printf("******!!%s",rel_name);
	FILE *disk = fopen("disk.txt","r");
	fseek(disk,4*2048+12, SEEK_SET);
	...
	//printf("%s",exp);
	FILE *fp_export=fopen(filename,"a");
	read_attr_cat(rel_name,no_attributes,filename);
